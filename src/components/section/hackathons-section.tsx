  ...
  return (
    <section id="hackathons" className="overflow-hidden">
      <div className="flex min-h-0 flex-col gap-y-8 w-full">
        ...
        <Timeline>
          {DATA.hackathons.map((hackathon: { title?: string; dates?: string; image?: string; location?: string; description?: string; links?: Array<{ href: string; title: string; icon?: any }> }) => (
            <TimelineItem key={(hackathon.title ?? '') + (hackathon.dates ?? '')} className="w-full flex items-start justify-between gap-10">
              <TimelineConnectItem className="flex items-start justify-center">
                {hackathon.image ? (
                  <div className="relative size-10 bg-card z-10 shrink-0 overflow-hidden p-1 border rounded-full shadow ring-2 ring-border flex-none">
                    <Image src={hackathon.image} alt={hackathon.title ?? ""} fill className="object-contain" />
                  </div>
                ) : (
